...
void main()
{
	int b[20][20],p[20],flag[20]={0},fragment[20],i,j,np,nb,allocation[20];
	clrscr();
	...
	for(i=1;i<=np;i++)
	{
		for(j=1;j<=nb;j++)
		{
			if(p[i]<=b[j][1])
			{
				if(flag[j]==0)
				{
					allocation[i]=b[j][0]; //allocation i stores blockno
...
printf("\nProcess no.\tPsize\t\tblockNO.\t\tsize\t\tfrags");
	for(i = 1; i <= np; i++)
	{
		printf("\n%d\t\t%d\t\t", i, p[i]);
		if(flag[i] == 1)
			printf("%d\t\t\t%d\t\t%d",allocation[i],b[allocation[i]][1],fragment[i]);
		else
